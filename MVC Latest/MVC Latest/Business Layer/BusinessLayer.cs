...
public class BusinessLayer : IBusinessAuthentication, IBusinessAccount
{
    ...
    public bool UpdatePassword(string uname, string oldpwd, string newpwd)
    {
        return _rap.UpdatePassword(uname, oldpwd, newpwd);

    }

    public bool RegisterUser(string uname, string password, string chkAcctNum)
    {
        return _rap.RegisterUser(uname, password, chkAcctNum);
    }
    ...
    {
        return _rap.TransferSavToChk(savAcctNum, chkAcctNum, amt);
    }

    public bool TransferFromChkgToSavViaSP(string chkAcctNum, string savAcctNum, double amt)
    {
        // return idac.TransferChkToSavViaSP(chkAcctNum,savAcctNum,amt);
        // return _rap.TransferChkToSavViaSP(chkAcctNum, savAcctNum, amt);
        return _rap.TransferChkToSavViaSP(chkAcctNum, savAcctNum, amt);  // mysql does not have the sp
    }
    #endregion

    /* public bool LoanSanction(string savAcctNum, string chkAcctNum, double amt)
     {
         return _rap.LoanSanction(savAcctNum, chkAcctNum, amt);
     }*/
    #region IBusinessAccount Members
    ...
    public string GetCheckingAccountNumber(string username)
    {
        ...
        try
        {
            ...
            res = _rap.GetCheckingAccountNumber(username);
        }
        catch (Exception)
        {
            throw;

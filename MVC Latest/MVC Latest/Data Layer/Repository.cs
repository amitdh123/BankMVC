...
public class Repository : IRepositoryDataAuthentication, IRepositoryDataAccount
{
   ...
   // public Repository(IDataAccess ida, CacheAbstraction webc)
    ...
    public bool TransferSavToChk(string savAcctNum, string chkAcctNum,
           double amt)
    {
        string CONNSTR = ConfigurationManager.ConnectionStrings["BANKDBCONN"].ConnectionString;
        bool res = false;
        ...
        {
            conn.Open();
            string sql = "SPXferSavToChk"; // name of SP
            SqlCommand cmd = new SqlCommand(sql, conn);
            SqlParameter p1 = new SqlParameter("@ChkAcctNum",
                                System.Data.SqlDbType.VarChar, 50);
    ...
    public bool RegisterUser(string uname, string pwd, string chkAcctNum)
    {
        bool RegisterUser = false;
        try
        {
            string sql = "INSERT INTO Users (Username, Password, CheckingAccountNum) VALUES (@uname , @pwd, @chkAcctNum)";
            List<DbParameter> Plist = new List<DbParameter>();
            SqlParameter p1 = new SqlParameter("@uname", SqlDbType.VarChar, 50);
            p1.Value = uname;
            Plist.Add(p1);
            ...
            p2.Value = pwd;
            Plist.Add(p2);
            SqlParameter p3 = new SqlParameter("@chkAcctNum", SqlDbType.VarChar, 50);
            p3.Value = chkAcctNum;
            Plist.Add(p3);
            int noOfRows = _dac.InsOrUpdOrDel(sql, Plist);
    ...
    public bool UpdatePassword(string uname, string oldPW, string newPW)
    {
        ...
        try
        {
            string sql = "Update Users set Password=@newPW where Username=@uname AND Password=@oldPW";
            List<DbParameter> Plist = new List<DbParameter>();
            SqlParameter p1 = new SqlParameter("@uname", SqlDbType.VarChar, 50);
            p1.Value = uname;
            Plist.Add(p1);
